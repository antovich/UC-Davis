%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%                                                                  %%
%%     Binned proportion of looking towards target by trial         %%
%%                                                                  %%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%                          
...
for irow = 1:nrows
    ...
    elseif obj2mask(irow) & obj2lab(irow)
        data.Target(irow) = 1;
    elseif obj2mask(irow) & obj1lab(irow)
        data.Target(irow) = 0;
    elseif obj1mask(irow) & obj2lab(irow)
        data.Target(irow) = 0;
    else data.Target(irow) = NaN;
    end
end
...
fid = fopen(fullfile(projFolder,[projName 'BinnedAvgTargVsDistByTrial.txt']),'at');
fprintf(fid,'SubID\tTrialStim\tCondition\tOrder\tInclusionStatus');
for ibins = 1:nBins
...
trialID = unique(data.Stimulus(testmask));
...
for isub = 1:nSub
    ...
    for itrial = 1:nTrial
        trialmask = strcmp(trialID{itrial}, data.Stimulus);
        fprintf(fid,'%s\t%s\t%s\t%d\t%s',subID{isub},trialID{itrial},SubInfo.Condition{subinfomask},SubInfo.Order(subinfomask),SubInfo.Inclusion_Status{subinfomask});
        for ibins = 1:nBins
            binsmask = data.Bin == bins(ibins);
            currmean = nanmean(data.Target(binsmask & submask & trialmask));
            fprintf(fid,'\t%g', currmean);
        end
        fprintf(fid,'\n');
    end
end
